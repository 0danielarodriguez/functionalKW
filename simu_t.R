...
for(mm in 1:lmedias)
{
  ...
  for(i in 1:nrep){
    print(i)
    set.seed(i)  
    X1 <- ModT(ene1, df=1, matrixphi,media0)
    X2 <- ModT(ene2, df=1, matrixphi,media)
    X3 <- ModT(ene3, df=1, matrixphi,media)
    X <- rbind(X1, X2, X3)
    ...
    write.table(pvalores, file = paste0("salidasR1/E1_t1","c_",cc10,"delta",delta,".txt"),  col.names = FALSE, row.names = FALSE, append = TRUE)
  }
}
