...
ces <- seq(0.1, 0.6, 0.1) #incluyo la nula 
delta0<-rep(0,lt)
...
for(mm in 1:lmedias)
{
  ...
  for(i in 1:nrep){
    print(i)
    set.seed(i)  
    X1 <- ModNormal(ene1, matrixphi,media0)
    X2 <- ModNormal(ene2, matrixphi,media)
    X3 <- ModNormal(ene3, matrixphi,media)
    X <- rbind(X1, X2, X3)
    ...
    pp <- fanova.tests(x = t(X), group.label = g, test = c("FP", "CH", "CS", "L2N", "L2B", "L2b", "FN", "FB", "Fb",  "TRP"))
    pvalores<-c(i,pkw,paov,p03,sapply(1:10, function(i)pp[[i]][2]) ,  pp[[10]][3])
    
    cc10<-cc*10
    write.table(pvalores, file = paste0("salidasR1/E1_normal","c_",cc10,"delta",delta,".txt"),  col.names = FALSE, row.names = FALSE, append = TRUE)
  }
}
